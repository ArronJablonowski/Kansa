...
$pattern = @('Pass Word', 'Password','passwd','root','UserName', 'User Name', 'login', 'passphrase', 'secret', '@yahoo', '@gmail', '@outlook', '@yourdomain', 'pwd', '/P')
...
function searchTextFiles() {
    Get-ChildItem -Recurse -file -path $searchPath -Include *.cmd, *.bat, *.batch, *.ps1, *.py, *.vbs, *.php, *.asp*, *.js, *.vba -Exclude *ClearTextPasswordsScripts.csv -ErrorAction $erroractionpref | 
