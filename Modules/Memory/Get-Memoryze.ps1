...
.SYNOPSIS
Get-Memoryze.ps1
    https://www.fireeye.com/services/freeware/memoryze-download-confirmation.html
...
if (Test-Path ($memoryzepath)) {
    $suppress = New-Item -Name Memoryze -ItemType Directory -Path $env:Temp -Force #places the file in the appdata temp folder 
    ...
    if (Test-Path($memoryze)) {
        & $cmdlocation  /c $memoryze |     #memorydd.bat gets called via cmd.exe 
                ForEach-Object { $_ }               
    } else {
