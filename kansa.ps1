...
Param(
    ...
    $Modules.Keys | Foreach-Object { $Module = $_
        ...
        $Job.ChildJobs | Foreach-Object { $ChildJob = $_
            $ModuleWarnings = @()
            $Recpt = Receive-Job $ChildJob -WarningVariable ModuleWarnings

            if ($ModuleWarnings) {
                $ModuleWarnings | ForEach-Object {
                    $ModuleName + " reports warning on " + $ChildJob.Location + ": `"" + $_ + "`"" | Add-Content -Encoding $Encoding $ErrorLog
                }
            }
